...
import { CheckCircle, Heart, Link, PlayCircleIcon, Trash2, Star } from 'lucide-react';
...
export default function VideoCard({
  ...
  const confirmDeleteRecord = useCallback(async () => {
    try {
      ...
      toast({
        title: '删除成功',
        description: `已删除「${actualTitle}」的播放记录`,
        variant: 'success',
      });
    } catch (err) {
      toast({
        title: '删除失败',
        description: '删除播放记录失败，请稍后重试',
        variant: 'destructive',
      });
      console.error('删除播放记录失败:', err);
    }
  }, [actualSource, actualId, onDelete, actualTitle, toast]);
  ...
  }, [actualSource, actualId, actualTitle, toast]);
  ...
  return (
    ...
    >
      ...
      <div className='relative aspect-[2/3] overflow-hidden rounded-lg'>
        ...
        {(config.showHeart || config.showCheckCircle) && (
          <div className='absolute top-2 right-2 flex gap-1 opacity-0 translate-y-2 transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:translate-y-0'>
            {config.showHeart && (
              <button
                onClick={handleToggleFavorite}
                className={`w-8 h-8 rounded-lg transition-all duration-300 ease-out hover:scale-[1.1] ${
                  favorited
                    ? 'bg-red-500/80 hover:bg-red-600/80 text-white'
                    : 'bg-white/20 hover:bg-white/30 text-white'
                }`}
              >
                <Heart className={`w-4 h-4 mx-auto ${favorited ? 'fill-current' : ''}`} />
              </button>
            )}
            {config.showCheckCircle && (
              <button
                onClick={handleDeleteRecord}
                className='w-8 h-8 bg-red-500/80 hover:bg-red-600/80 text-white rounded-lg transition-all duration-300 ease-out hover:scale-[1.1]'
              >
        ...
        {config.showRating && rate && (
          <div className='absolute top-2 left-2 bg-yellow-500 text-white text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-1 shadow-md transition-all duration-300 ease-out group-hover:scale-110 z-10'>
        ...
        {actualEpisodes && actualEpisodes > 1 && (
          <div className='absolute bottom-2 left-2 bg-blue-500 text-white text-xs font-semibold px-2 py-1 rounded-md shadow-md transition-all duration-300 ease-out group-hover:scale-110'>
            {currentEpisode
    ...
    <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>确认删除播放记录</DialogTitle>
          <DialogDescription>
            确定要删除「{actualTitle}」的播放记录吗？
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
    ...
    <Dialog open={showUnfavoriteDialog} onOpenChange={setShowUnfavoriteDialog}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            确定要取消收藏「{actualTitle}」吗？
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="outline" onClick={() => setShowUnfavoriteDialog(false)}>
            保留收藏
          </Button>
          <Button variant="destructive" onClick={confirmUnfavorite}>
            确定取消
          </Button>
        </DialogFooter>
      </DialogContent>
